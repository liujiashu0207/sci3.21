...
run2 = p.add_run(
    ...
    '有效减小路径长度并降低转弯次数。在MovingAI公开基准数据集的15张静态栅格地图上，以20条任务×7种算法进行'
    '对比实验，在本文实验条件下观察到：与A*(欧氏)相比，RDA*的路径长度缩短3.6%（p<0.001），转弯次数减少76.4%'
    '（p<0.001），扩展节点减少61.5%（p<0.001），总运行时间缩短31.9%；与A*(Octile)相比，扩展节点减少50.9%，转弯'
    '次数减少46.4%，均达到统计显著水平。消融实验表明自适应权重与路径平滑两个模块各自具有独立贡献。补充实验在'
    '长路径场景下表现出一致的改进趋势。上述结论适用于本文所采用的静态栅格环境与基线配置。'
)
...
run2 = p.add_run(
    ...
    'while maintaining conservative search in dense regions. A two-stage path smoothing strategy effectively reduces path length '
    'and turn count while ensuring path legality. Experiments on 15 static grid maps from the MovingAI benchmark dataset with '
    '20 tasks × 7 algorithms indicate that, under the experimental conditions of this study, compared with A*(Euclidean), RDA* '
    'reduces path length by 3.6% (p<0.001), turn count by 76.4% (p<0.001), expanded nodes by 61.5% (p<0.001), and total runtime '
    'by 31.9%; compared with A*(Octile), expanded nodes are reduced by 50.9% and turn count by 46.4%, both statistically '
    'significant. Ablation studies indicate the independent contributions of the adaptive weight and path smoothing modules. '
    'These findings are specific to the static grid environments and baseline configurations used in this study.'
)
run2.font.size = Pt(9)
run2.font.name = 'Times New Roman'
...
add_para(
    ...
    '平滑，在保证路径合法性的前提下有效减小路径长度并降低转弯次数。'
)
add_para(
    '（3）在MovingAI公开基准数据集的15张地图上，采用7种算法（含2组消融）×20条任务的大规模实验设计，结合Wilcoxon'
    '符号秩检验和BH-FDR校正进行统计分析，对算法的有效性进行了较为充分的实验验证。'
)
...
add_para(
    ...
    '查询O(1)；A*搜索O(N\'·logN\')，其中N\'为改进后的扩展节点数，明显小于传统A*的N；路径简化O(n·L)，其中L为视线'
...
add_para(
    '由表4可知，在本文实验条件下，与A*(欧氏)相比，RDA*算法在路径长度、转弯次数、扩展节点和总运行时间四项指标上'
    '均表现出改进趋势：路径长度缩短3.6%，转弯次数减少76.4%，扩展节点减少61.5%，总运行时间缩短31.9%。'
)

add_para(
    '值得注意的是，A*(欧氏)在八连通栅格中并非最强基线。A*(Octile)采用更紧凑的Octile启发函数，扩展节点降至13331'
    '（比A*(欧氏)少21.7%），转弯次数降至22.5。即便以A*(Octile)为参照，RDA*仍将扩展节点进一步减少50.9%（p<0.001，'
    'W/T/L=15/0/0），转弯次数减少46.4%（p<0.001，W/T/L=15/0/0），总运行时间缩短24.2%（p=0.041）。与WA*(1.2)的对比'
    '则体现了不同的权衡取向：WA*(1.2)在搜索速度上优于RDA*（total_ms 60.49 vs 86.04），但其路径质量明显逊色（PL '
    '318.53 vs 301.76，TC 37.5 vs 12.1），且扩展节点多8.7%。RDA*通过自适应权重与后处理平滑的协同，在路径质量与'
    '搜索开销之间取得了更优的综合权衡。'
)
...
add_para(
    'PL、TC、EXP三项核心指标的q_bh值均远小于0.001，W/T/L=15/0/0，即在本文所设定的15张地图与任务配置下，RDA*的'
    '三项核心指标均优于A*(欧氏)。需要指出的是，本文主结论并非仅基于A*(欧氏)一种基线，表4中A*(Octile)和WA*(1.2)'
    '的对照结果同样支持上述改进趋势——RDA*相对A*(Octile)在EXP和TC上的改进幅度分别为50.9%和46.4%，均达到统计'
    '显著（q_bh<0.001）。'
)
...
add_para(
    ...
    '如表7所示。补充实验的改进趋势与主实验保持一致，三项核心指标的p值均小于0.001，W/T/L均为15/0/0。长路径场景下'
    '各指标的改进幅度略有降低（如EXP −58.4% vs −61.5%），但差异较小，表明RDA*在本文测试的困难场景范围内表现出较好'
    '的稳定性。'
)
...
add_para(
    ...
    '残差驱动自适应加权A*路径规划算法（RDA*），通过融合局部障碍物密度感知与两阶段路径平滑策略，在搜索效率与'
    '路径质量之间寻求更优的权衡。'
)
...
add_para(
    '实验验证方面，本文在MovingAI公开基准数据集的15张静态栅格地图上进行了系统性实验。主实验以7种算法×20条任务'
    '共计2100次搜索为基础，辅以Wilcoxon符号秩检验和BH-FDR校正进行统计分析。在本文实验条件下观察到，与A*(欧氏)'
    '相比，RDA*的路径长度缩短3.6%，转弯次数减少76.4%，扩展节点减少61.5%，总运行时间缩短31.9%，四项指标均达到'
    '统计显著水平。即便以更强的A*(Octile)为基线，RDA*在扩展节点和转弯次数上的改进仍分别达到50.9%和46.4%（均'
    'p<0.001）。消融实验表明自适应权重模块与路径平滑模块各自具有独立贡献：前者使扩展节点减半，后者使路径长度'
    '缩短8.1%、转弯次数减少79.2%，两个模块功能正交、互为补充。补充实验在长路径场景下表现出一致的改进趋势。'
)

add_para(
    '需要指出的是，本文贡献的核心在于路径质量与搜索开销的综合权衡改进，而非所有单项指标的绝对最优。例如，WA*(1.2)'
    '在搜索速度上优于RDA*，但其路径质量明显逊色。RDA*通过自适应权重与后处理平滑的协同，在多项指标间取得了更优的'
    '平衡点。'
)

add_para(
    '同时，本文也存在以下局限：（1）上述实验结论适用于MovingAI基准数据集的静态栅格环境，在动态障碍、连续空间或三维'
    '场景中的表现尚未验证；（2）在障碍物分布均匀的环境中，局部障碍率的空间差异较小，自适应权重退化为近似固定值，'
    '算法优势减弱；（3）当前实验仅在仿真环境中进行，尚未在实际机器人平台上验证算法的实时性和可行性。'
)
