...
bpara('摘　要：',
    ...
    '策略（视线简化与角点插值），在保证路径合法性的前提下有效缩短路径长度并减少转弯次数。在MovingAI公开基准数据集'
    '的15张地图上，以20条任务×7种算法进行对比实验，结果表明：与A*(欧氏)相比，RDA*的路径长度缩短3.6%（自适应权重'
    '与两阶段平滑的综合效果，p<0.001），'
    '转弯次数减少76.4%（p<0.001），扩展节点减少61.5%（p<0.001），总运行时间缩短31.9%。其中，扩展节点与搜索时间的'
    '改进主要来自自适应权重模块，路径长度与转弯次数的改进主要来自两阶段平滑模块。补充实验在长路径场景下表明上述'
    '改进趋势具有一致性。上述结论在静态栅格环境与MovingAI基准数据集的实验条件下成立，向动态障碍等场景的推广有待'
    '进一步验证。')
...
r2=p.add_run(
    ...
    'maintaining conservative search in dense regions. A two-stage path smoothing strategy effectively reduces path length '
    ...
    '(p<0.001), and total runtime by 31.9%. Ablation studies indicate the independent contributions of the adaptive weight and '
    'path smoothing modules. The above conclusions hold under static grid environments with the MovingAI benchmark.')
...
para('（2）设计两阶段路径平滑策略。第一阶段基于严格supercover视线检测的路径简化，第二阶段基于角点加权插值的路径'
     '平滑，在保证路径合法性的前提下有效缩短路径长度并减少转弯次数。')
para('（3）在MovingAI公开基准数据集的15张地图上，采用7种算法（含2组消融）×20条任务的大规模实验设计，结合Wilcoxon'
     '符号秩检验和BH-FDR校正进行统计分析，对算法的有效性和鲁棒性进行了实验验证。')
...
para('其中，n为当前节点；g(n)为从起点到节点n的实际代价；h(n)为从节点n到目标点的估计代价（启发函数）。A*算法通过'
     ...
     '中，欧氏距离虽然满足可接受性，但会低估对角移动的代价，导致扩展大量冗余节点。本文采用八连通邻域搜索，'
     '对角移动采用AND逻辑约束，即仅当对角方向的两个正交邻居均为自由栅格时才允许对角移动，避免路径斜穿障碍物栅格。'
     '正交移动步长为1，对角移动步长为√2。')
...
para('其中，dx=|x_n−x_g|，dy=|y_n−y_g|分别为当前节点与目标点在x、y方向上的距离差。Octile距离精确描述了八连通网格'
     '中的最短移动距离，相比欧氏距离能更准确地估计实际路径代价，从而减少冗余节点的扩展。需要指出的是，1.1节所述的'
     '对角移动AND约束仅禁止穿越障碍物的对角移动，不会改变无障碍区域中八连通栅格的最短路径代价，因此Octile距离在'
     '该约束下仍满足启发函数的可接受性条件h_oct(n)≤h*(n)。')
...
para('其中，grid(i,j)∈{0,1}为栅格地图的占用值，0表示自由栅格，1表示障碍物栅格；(2r+1)²为窗口内的栅格总数。由定义'
     ...
     '窗口大小为11×11，该参数的选择论证见1.3.4节。当节点位于地图边缘，窗口部分超出地图边界时，仅对窗口内位于地图'
     '范围内的有效栅格进行统计，分母相应取有效栅格数，避免越界导致的密度估计偏差。')
...
para('通行残差r_res(n)具有以下边界性质：（1）当r_res(n)→1时，节点周围几乎无障碍物，通行裕度充足，搜索空间开阔，'
     '此时存在大量等价或近似等价的路径，标准A*算法会扩展大量冗余节点，搜索效率较低；（2）当r_res(n)→0时，节点周围障碍物密集，'
...
para('上述两阶段平滑策略输出的是无碰撞的折线路径，适用于全向移动机器人。对于差分驱动等具有非完整运动学约束的'
     '机器人，可在本文平滑结果的基础上，进一步结合B样条曲线、Dubins曲线等方法进行运动学适配。')

# 1.4.3 算法伪代码与流程
heading('1.4.3　算法整体流程', 3)
para('RDA*算法的完整流程分为预处理、搜索和后处理三个阶段，伪代码如下：')

# 伪代码用等宽字体
pseudocode = [
    'Algorithm: RDA* (Residual-Driven Adaptive Weighted A*)',
    'Input:  grid (H x W), start s, goal g, beta=0.3, r=5',
    'Output: smoothed path P',
    '',
    '// Stage 1: Preprocessing',
    '1  I <- BuildIntegralImage(grid)          // O(H x W)',
    '',
    '// Stage 2: Adaptive weighted A* search',
    '2  open <- {s};  closed <- {};  g(s) <- 0',
    '3  while open is not empty do',
    '4      n <- ExtractMin(open)              // min f(n)',
    '5      closed <- closed + {n}',
    '6      if n = goal then break',
    '7      for each neighbor nb of n do',
    '8          cost <- StepCost(n, nb)        // 1 or sqrt(2)',
    '9          if nb in closed then continue',
    '10         g_new <- g(n) + cost',
    '11         if g_new < g(nb) then',
    '12             g(nb) <- g_new',
    '13             parent(nb) <- n',
    '14             rho <- QueryIntegral(I, nb, r)  // O(1)',
    '15             r_res <- 1 - rho',
    '16             alpha <- 1 + beta * r_res',
    '17             f(nb) <- g(nb) + alpha * h_oct(nb, goal)',
    '18             Insert(open, nb, f(nb))',
    '19 P0 <- BacktracePath(parent, goal)',
    '',
    '// Stage 3: Two-stage smoothing',
    '20 P1 <- LineOfSightSimplify(P0, grid)   // supercover LOS',
    '21 P  <- CornerSmooth(P1, grid)           // weighted interp',
    '22 return P',
]
for line in pseudocode:
    ...
    r.font.size = Pt(8.5)
    if line.startswith('Algorithm') or line.startswith('Input') or line.startswith('Output') or line.startswith('//'):
        r.bold = True

para('算法整体流程如图1所示，分为预处理阶段（构建积分图）、A*搜索阶段（自适应权重驱动的节点扩展）和后处理阶段'
     '（两阶段路径平滑）。其中，搜索阶段的核心改进在于第14-16行：对每个邻居节点通过积分图查询局部障碍率（O(1)），'
     '计算通行残差r_res，并据此动态调整启发权重alpha，使搜索在空旷区域加速、在密集区域保守。')

img('figures/algorithm_flowchart.png', 10)
caption('图1　RDA*算法流程图', 'Fig.1　Flowchart of RDA* algorithm')
...
heading('1.5.2　路径代价上界分析', 3)
para('本节分析RDA*算法搜索阶段（平滑前）输出路径的代价上界。RDA*的代价函数为f(n)=g(n)+α(n)·h_oct(n)，其中'
     'α(n)∈[1, 1+β]。令w_max=1+β=1.3，则对任意节点n有：')
formula('α(n) · h_oct(n) ≤ w_max · h_oct(n)', '（9）')
para('因此，RDA*的代价函数满足：')
formula('f_RDA(n) ≤ g(n) + w_max · h_oct(n) = f_WA(n)', '（10）')
para('其中f_WA(n)为使用固定权重w_max的标准加权A*的代价函数。根据加权A*算法的经典理论[5]，当启发函数满足可接受性'
     '时，固定权重w_max的WA*算法返回的路径代价C_WA满足C_WA≤w_max·C*。由于RDA*对每个节点的f值不超过WA*对同一节点'
     '的f值（式（10）），参照上述经典结论，RDA*搜索阶段返回的路径代价C_search的理论上界约为：')
formula('C_search ≤ (1+β) · C* = 1.3 · C*', '（11）')
para('即搜索阶段的路径长度最多比最优路径增加约30%。实验结果表明，β=0.3时，RDA*搜索阶段的路径长度（即NoSmooth消融'
     '变体的输出）仅比A*(Octile)的最优路径增加4.9%（328.41 vs 312.94），远优于30%的理论上界。经过两阶段平滑后，最终'
     '路径长度进一步缩短至301.76，反而优于基线，说明平滑策略有效补偿了加权搜索带来的路径次优性。')
para('需要说明的是，上述上界分析基于将变权重搜索与固定权重加权A*进行对比的工程近似推导，用于刻画算法的有界次优'
     '趋势。变权重自适应A*的严格次优性上界数学证明涉及节点扩展顺序的精确分析，将作为后续的理论研究方向。')
...
para('实验在Ubuntu 24.04操作系统下进行，编程语言为Python 3.12，硬件环境为Intel Core i9-4900HX处理器、16 GB DDR4内存，'
     '所有算法实验均在单线程下执行，避免多线程调度对运行时间结果的干扰。本文采用MovingAI公开基准数据集[4]进行实验'
     '验证。'
...
para('采用路径长度（PL）、转弯次数（TC）、扩展节点数（EXP）、搜索时间（search_ms）、总时间（total_ms）和碰撞检测（CF）'
     '6项指标评价算法性能。对每张地图取20条任务的均值作为该地图的代表值，得到n=15个配对样本。由于样本量较小且无法'
     '假设指标差值服从正态分布，采用非参数的Wilcoxon符号秩检验（双侧）进行配对比较。因同时对多项指标进行检验，使用'
     'Benjamini-Hochberg方法进行FDR校正以控制假阳性率。同时报告Cohen\'s d效应量和Win/Tie/Loss统计。')
...
caption('图2　β参数敏感性分析', 'Fig.2　β parameter sensitivity analysis')
...
para('由表6可知，与A*(欧氏)相比，本文RDA*算法在各核心指标上均表现出改进：路径长度缩短3.6%，转弯次数减少76.4%，'
     '扩展节点减少61.5%，总运行时间缩短31.9%。其中，路径长度的缩短为自适应权重模块与两阶段平滑模块的综合效果：'
     '自适应权重使搜索阶段路径长度较标准A*有小幅增加，而两阶段平滑对此进行了补偿并进一步优化，各模块的具体贡献'
     '见3.4节消融分析。')

para('进一步对比强基线可以发现，相对A*(Octile)，RDA*的扩展节点减少50.9%（6550 vs 13331），路径长度缩短3.6%（301.76 '
     'vs 312.94），其中扩展节点的减少并非仅源于启发函数从欧氏距离到Octile距离的替换，自适应权重模块提供了核心的加速'
     '贡献。相对WA*(1.2)，RDA*的路径长度更短（301.76 vs 318.53）、转弯次数更少（12.1 vs 37.5）、扩展节点更少（6550 '
     'vs 7171），但搜索时间略长（80.67 ms vs 60.49 ms），原因是RDA*需要逐节点查询局部障碍率。综合来看，RDA*以少量'
     '搜索时间开销换取了路径质量与搜索效率的更优权衡。')
img('figures/chart_expanded_nodes.png', 14)
caption('图3　15张地图扩展节点数对比', 'Fig.3　Comparison of expanded nodes across 15 maps')
img('figures/chart_path_length.png', 14)
caption('图4　15张地图路径长度对比', 'Fig.4　Comparison of path length across 15 maps')
img('figures/chart_turn_count.png', 14)
caption('图5　15张地图转弯次数对比', 'Fig.5　Comparison of turn count across 15 maps')
...
para('需要指出的是，A*(Octile)作为采用最紧凑可接受启发函数的基线，其路径长度与A*(欧氏)一致（均为312.94），但扩展'
     '节点更少（13331 vs 17019）。由表6可知，RDA*相对A*(Octile)仍表现出改进：扩展节点减少50.9%（6550 vs 13331），'
     '转弯次数减少46.4%（12.1 vs 22.5），路径长度缩短3.6%（301.76 vs 312.94），总运行时间缩短24.2%（86.04 vs '
     '113.43）。这表明扩展节点的减少并非仅来自启发函数从欧氏距离到Octile距离的替换，自适应权重模块在Octile基线之上'
     '提供了核心的搜索加速贡献；路径长度与转弯次数的改进则主要来自两阶段平滑模块。')

para('综合以上分析，本文的主要结论不仅基于与A*(欧氏)的统计检验，同时参考了A*(Octile)和WA*(1.2)的对照结果，以确保'
     '结论在不同强度的基线下均成立。')
...
para('为直观展示自适应权重对搜索范围的缩减效果，图6以bloodvenomfalls地图为例，可视化了三种算法扩展过的节点分布。'
     ...
     '范围几乎完全相同（均为12905个节点），而RDA*的搜索范围明显缩小（仅2057个节点，减少84.1%），直观展示了自适应'
     '权重机制对搜索效率的贡献。')
...
tbl(['指标','主实验(PL≈316)','补充实验(PL≈626)'],
    [['PL变化率','−3.6% ***','−2.8% ***'],
     ['TC变化率','−76.4% ***','−71.9% ***'],
     ['EXP变化率','−61.5% ***','−58.4% ***']])
para('补充实验的改进趋势与主实验一致，三项核心指标的p值均小于0.001，表明RDA*在长路径场景下仍表现出稳定的改进效果。')

# ═════════════ 4 结论 (r_res符号) ═════════════
heading('4　结论', 1)
para('本文针对传统A*算法在复杂栅格环境中搜索效率受限、路径冗余转折多、启发函数权重无法自适应调整等问题，提出了'
     '残差驱动自适应加权A*路径规划算法（RDA*），通过融合局部障碍物密度感知与两阶段路径平滑策略，在搜索效率与路径'
     '质量之间取得了更优的权衡。')
...
para('在路径优化方面，本文提出两阶段平滑策略：第一阶段基于严格supercover视线检测进行路径简化，去除冗余中间节点；'
     ...
     '穿越任何障碍物。该策略在本文实验设定（静态栅格、MovingAI基准、β=0.3、r=5）下有效补偿了加权搜索带来的路径'
     '次优性，使最终路径长度相对标准A*基线表现出改进。')
para('实验验证方面，本文在MovingAI公开基准数据集的15张地图上进行了对比实验。主实验以7种算法×20条任务共计2100次搜索'
     '为基础，辅以Wilcoxon符号秩检验和BH-FDR校正进行统计分析。在本文设定的实验条件下，与A*(欧氏)相比，RDA*的路径'
     '长度缩短3.6%，转弯次数减少76.4%，扩展节点减少61.5%，总运行时间缩短31.9%，上述差异均达到统计显著水平。消融'
     '实验表明自适应权重模块与路径平滑模块各自具有独立贡献：前者使扩展节点减半，后者使路径长度缩短8.1%、转弯次数'
     '减少79.2%，两个模块功能正交、互为补充。补充实验在长路径场景（平均最优路径≈626步）下观察到一致的改进趋势。')
para('需要指出的是，本文的贡献定位为路径质量与搜索开销的综合权衡改进，而非所有指标的绝对最优——例如WA*(1.2)的搜索'
     '时间更短，NoAdaptive的路径长度更短，但RDA*在路径长度、转弯次数、扩展节点三项核心指标上取得了更均衡的表现。'
     '同时，本文也存在以下局限：（1）上述结论在静态栅格环境、MovingAI基准数据集、当前参数（β=0.3, r=5）与基线配置下'
     '成立，向动态障碍、三维空间等场景的推广有待进一步验证；（2）在障碍物分布均匀的环境中，局部障碍率的空间差异较小，'
     '自适应权重退化为近似固定值，算法优势减弱；（3）当前实验仅在仿真环境中进行，尚未在实际机器人平台上验证；'
     '（4）当前平滑策略输出折线路径，未考虑差分驱动等机器人的非完整运动学约束，后续将针对不同类型的机器人平台优化'
     '平滑策略以满足实际运动约束。')
...
out = '/mnt/user-data/outputs/RDA_star_论文修改稿_v6.1.docx'
doc.save(out)
print(f"✅ {out}")
