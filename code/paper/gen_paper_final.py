...
bpara('摘　要：',
    ...
    '与两阶段平滑的综合效果，p<0.001），'
    '转弯次数减少76.4%（p<0.001），扩展节点减少61.5%（p<0.001），总运行时间缩短31.9%；消融实验表明自适应权重'
    '与路径平滑两个模块各自具有独立贡献。补充实验在长路径场景下表明上述改进趋势具有一致性。上述结论在静态栅格'
    '环境与MovingAI基准数据集的实验条件下成立，向动态障碍等场景的推广有待进一步验证。')
...
para('进一步对比强基线可以发现，相对A*(Octile)，RDA*的扩展节点减少50.9%（6550 vs 13331），路径长度缩短3.6%（301.76 '
     'vs 312.94），表明改进并非仅源于启发函数的替换。相对WA*(1.2)，RDA*的路径长度更短（301.76 vs 318.53）、转弯次数'
     '更少（12.1 vs 37.5）、扩展节点更少（6550 vs 7171），但搜索时间略长（80.67 ms vs 60.49 ms），原因是RDA*需要逐'
     '节点查询局部障碍率。综合来看，RDA*以少量搜索时间开销换取了路径质量与搜索效率的更优权衡。')
...
para('需要指出的是，A*(Octile)作为采用最紧凑可接受启发函数的基线，其路径长度与A*(欧氏)一致（均为312.94），但扩展'
     ...
     '113.43）。这表明RDA*的性能改进并非仅来自启发函数从欧氏距离到Octile距离的替换，自适应权重机制和两阶段平滑在'
     'Octile基线之上仍提供了进一步的改进。')
...
out = '/mnt/user-data/outputs/RDA_star_论文修改稿_v5.6.docx'
doc.save(out)
print(f"✅ {out}")
